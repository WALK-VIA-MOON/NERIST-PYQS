...
const deptConfig = {
  cse: {
    ...
    academicYears: [
      {
        key: "btech1st",
        label: "B.Tech 1st Year",
        papers: ["2025", "2024", "2023", "2022"],
      },
      {
        key: "btech2nd",
...
const driveLinks = {
  "cse-btech1st-2025-mid":
    "https://drive.google.com/drive/folders/1Vno2ZWB8SGUmeBr-J7tlA9EJ6f7AOubY?usp=drive_link",
...
function renderAcademicYears(dept, section) {
  ...
  config.academicYears.forEach((y) => {
    ...
    div.addEventListener("click", () =>
      renderPaperYears(dept, y.key, y.papers, section)
    );
...
function renderPaperYears(dept, academicKey, paperYears, section) {
  ...
  paperYears.forEach((yr) => {
    ...
    div.textContent = yr;
    div.addEventListener("click", () =>
      renderExamTypes(dept, academicKey, yr, section)
    );
    grid.appendChild(div);
  });
